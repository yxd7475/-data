...
def image_to_base64(image, format='png'):
    ...
    # 使用PNG保持最高质量，压缩级别设为0（无损）
    if format == 'png':
        _, buffer = cv2.imencode(f'.{format}', image, [cv2.IMWRITE_PNG_COMPRESSION, 0])
    else:
        # JPEG使用最高质量
        _, buffer = cv2.imencode(f'.{format}', image, [cv2.IMWRITE_JPEG_QUALITY, 100])
