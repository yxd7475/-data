...
class DocumentScanner:
    ...
    def enhance_document(self, image: np.ndarray, mode: str = 'scanner') -> np.ndarray:
        ...
        if mode == 'scanner':
            ...
            cdf = hist.cumsum()
            total = cdf[-1]
            white_threshold = np.searchsorted(cdf, total * 0.95)

            if white_threshold < 255:
                bright_mask = normalized >= white_threshold
                if np.sum(bright_mask) > 100:
                    bg_mean = np.mean(normalized[bright_mask])
                    # 将背景映射到255
                    scale = 255.0 / bg_mean if bg_mean > 200 else 1.2
                    normalized = np.clip(normalized.astype(np.float32) * scale, 0, 255)
            ...
            # 4. 混合：二值化背景 + 原始文字细节
            # 在二值化背景上叠加增强后的文字
            result_gray = binary.copy()

            # 5. 对比度增强 - 让文字更深
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            enhanced_gray = clahe.apply(normalized)

            # 在暗色区域（文字）使用增强后的灰度，保留层次
            dark_mask = enhanced_gray < 180
            result_gray[dark_mask] = enhanced_gray[dark_mask]
            ...
            # 对亮度做同样的背景归一化
            l_bg = cv2.GaussianBlur(l, (kernel_size, kernel_size), 0)
            l_norm = cv2.divide(l, l_bg, scale=255)
            l_enhanced = clahe.apply(l_norm)

            lab_result = cv2.merge([l_enhanced, a, b])
            color_result = cv2.cvtColor(lab_result, cv2.COLOR_LAB2BGR)

            # 8. 混合输出
            result = cv2.cvtColor(result_gray, cv2.COLOR_GRAY2BGR)
            result[color_mask] = color_result[color_mask]
